...
class LifeViewer():
    def __init__(self, x, y, life_amount=5, move_ratio = [0,0]):
        self.x = x
        self.y = y
        self.move_ratio = move_ratio
        self.full_life = life_amount
        self.life = life_amount
        self.game_over = False

        self.img_size = 20
        self.life_image_list = [Image(self.x + self.img_size*i, self.y, "HP", size=[self.img_size,self.img_size]) for i in range(self.full_life)]
    ...
    def move_to(self, dx,dy):
        ...
        # # change rect pos
        # for img in self.life_image_list:
        #     img.move_image(dx,dy)
    ...
    def draw(self,screen):
        if self.turn_on:
            cnt = 0
            for life in self.life_image_list:
                if cnt < self.life:
                    life.draw(screen)
                cnt += 1
